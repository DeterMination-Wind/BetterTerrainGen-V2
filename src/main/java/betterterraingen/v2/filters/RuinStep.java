...
/** One ordered floor/wall operation around a generated ruin center. */
public class RuinStep implements Serializable {
    ...
    public RuinStep copy() {
        ...
        if (step.wall != null && step.wall == Blocks.removeWall) {
            step.wall = null;
    ...
    public boolean removesWall() {
        return removeWall || wall != null && wall == Blocks.removeWall;
    }

    public boolean matches(int centerX, int centerY, int x, int y) {
        if (!Float.isFinite(radius) || radius < 0f) return false;

        float dx = x - centerX;
        float dy = y - centerY;
        StepMode mode = stepMode == null ? StepMode.chebyshev : stepMode;
        return switch (mode) {
            case geometric -> Math.sqrt(dx * dx + dy * dy) <= radius;
            case manhattan -> Math.abs(dx) + Math.abs(dy) <= radius;
            case chebyshev -> Math.max(Math.abs(dx), Math.abs(dy)) <= radius;
        };
    }
    ...
    public static RuinStep[] defaults() {
        return new RuinStep[]{
            floor(6f, Blocks.metalFloor),
            floor(1f, Blocks.metalFloor3),
            wall(4f, Blocks.metalWall2),
            removeWall(3f)
        };
    }
